from auto_dpgan.rdp_accountant import compute_rdp, get_privacy_spent


def epsilon(n_samples, batch_size, noise_multiplier, iterations, delta=1e-5):
    """Calculates epsilon for stochastic gradient descent.

    Args:
        n_samples (int): Total numbers of examples
    ...
    optimal_order = _ternary_search(
        lambda order: _apply_dp_sgd_analysis(q, noise_multiplier, iterations,
                                             [order], delta), 1, 512, 72)
    return _apply_dp_sgd_analysis(q, noise_multiplier, iterations,
                                  [optimal_order], delta)
...
def _apply_dp_sgd_analysis(q, sigma, iterations, orders, delta):
    ...
    Args:
        q (float): Sampling probability, generally batch_size / n_samples
...
def _ternary_search(f, left, right, iterations):
    """Performs a search over a closed domain [left, right] for the value
    which minimizes f.
    """
